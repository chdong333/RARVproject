...
改进版：修复 E/A None 导致崩溃、修正 BSA 列识别、峰识别更稳健、增加日志
保持原输出结构与每4行一例逻辑不变。
"""
import os, re, json
import numpy as np
...
PEAK_MIN_PROM_FRAC    = 0.02  # 峰显著性相对于 max(|drv|) 的比例（双阈值）
DPI = 150
...
def _savgol(y: np.ndarray, win_frac: float, poly: int) -> np.ndarray:
    ...
    if w < 7:
        return y
    return savgol_filter(y, window_length=w, polyorder=min(poly, w-1), mode="wrap")
...
def resample_with_periodic_spline(vols: np.ndarray,
    ...
    N = vols.shape[0]
    if N <= 0:
        return np.linspace(0.0,1.0,upsample_M,endpoint=False), np.full(upsample_M, np.nan), np.full(upsample_M, np.nan)
    ...
    try:
        ...
        # 拟合失败，退化为线性重复
        y_repeat = np.tile(vols.mean(), upsample_M)
        return np.linspace(0.0,1.0,upsample_M,endpoint=False), y_repeat, np.zeros_like(y_repeat)
...
def find_EA_windowed(drv: np.ndarray, idx_ES: int, idx_ED: int, M: int,
    ...
    无峰时返回 (None, None)（上层负责降级为 NaN）。
    """
    seg = forward_interval(idx_ES, idx_ED, M)
    if len(seg) == 0:
        return None, None
    drv_seg = drv[seg]
    # 保护：若 drv 全为 NaN 或幅值极小，则不找峰
    if not np.isfinite(drv_seg).any():
        return None, None
    std_val = float(np.nanstd(drv_seg))
    max_abs = float(np.nanmax(np.abs(drv_seg)))
    if max_abs < 1e-8:
        return None, None

    min_distance = max(3, int(M * (min_dist_pct / 100.0)))
    prom_from_std = std_val * prom_std
    prom_from_max = max_abs * PEAK_MIN_PROM_FRAC
    prom = max(1e-9, float(max(prom_from_std, prom_from_max)))

    peaks_rel, props = find_peaks(drv_seg, distance=min_distance, prominence=prom)
    if len(peaks_rel) == 0:
        return None, None

    # 转换回全局索引并筛除边界（edge_buffer）
    peaks = []
    ...
    def _pick(win, earliest=True):
        lo, hi = win
        cands = [(p, pr) for (p, pr) in peaks if lo <= pr <= hi]
    ...
    # 兜底：若某窗无峰，选择窗外最接近中心的峰
    if iE is None:
        center = (e_win[0] + e_win[1]) / 2.0
        iE = min(peaks, key=lambda x: abs(x[1]-center))[0]
    if iA is None:
        centerA = (a_win[0] + a_win[1]) / 2.0
        iA = min(peaks, key=lambda x: abs(x[1]-centerA))[0]

    # 强制 A 晚于 E（若不满足则把 A 设为 None，让上层采用退化策略）
    prE = _progress_in_arc(iE, idx_ES, idx_ED, M)
    prA = _progress_in_arc(iA, idx_ES, idx_ED, M)
    if prA <= prE:
        return iE, None
...
def find_eject_windowed(drv: np.ndarray, idx_ED: int, idx_ES: int, M: int,
    ...
    if cand:
        ...
        return int(arr[np.argmin(drv[arr])])
    # 再在除边界的全段
    cand2 = [k for (k, pr) in prog if edge_buffer <= pr <= 1.0 - edge_buffer]
    if cand2:
        arr2 = np.array(cand2, dtype=int)
        return int(arr2[np.argmin(drv[arr2])])
    # 最后全段
    return int(seg[np.argmin(drv_seg)])
...
def load_bsa_table(bsa_path: str) -> Optional[pd.DataFrame]:
    if not bsa_path or not os.path.exists(bsa_path):
        print("[BSA] 未找到BSA文件，将跳过体积指数化。"); return None
    ...
    for c in df.columns:
        cstr = str(c).strip()
        if cstr in ["影像号","PatientID","CaseID","ID","编号"] or "影像" in cstr or "patient" in cstr.lower():
            id_col = c; break
    if id_col is None:
        # 兜底第1列
        id_col = df.columns[0]
        print(f"[BSA] 未显式找到影像号列，使用第1列：{id_col}")
    ...
    if bsa_col is not None and df[bsa_col].notna().any():
        ...
        if h_col is None or w_col is None:
            print("[BSA] BSA表缺少 BSA 或 身高/体重列，跳过指数化。"); return None
...
def compute_right_metrics(patient_id: str,
    ...
    if i_E is not None:
        ...
        RV_E_vel_ml_perc, t_RV_E_pct = np.nan, np.nan
    if i_A is not None:
    ...
    if i_A is not None:
        ...
        frac_idx = int(0.8 * (len(idx_fill) - 1)) if len(idx_fill)>0 else 0
    ...
    peak_eject_diff = wrap_pct((i_eject - i_lv_eject) / M * 100.0) if (i_eject is not None and i_lv_eject is not None) else np.nan
    i_lv_E, _ = find_EA_windowed(d_LV, i_LV_ES, i_LV_ED, M,
                                 e_win=(0.05, 0.35), a_win=(0.55, 0.95))
    peak_fillE_diff = wrap_pct((i_E - i_lv_E) / M * 100.0) if (i_E is not None and i_lv_E is not None) else np.nan
    ...
    res.update({
        ...
        "RA_RV_coupling_total": (RA_reservoir_EF/RVEF) if (RVEF is not None and not np.isnan(RVEF) and RVEF != 0) else np.nan,
        "RA_RV_coupling_active": (RA_active_EF/RVEF) if (RVEF is not None and not np.isnan(RVEF) and RVEF != 0) else np.nan,
...
def compute_left_metrics(patient_id: str,
    ...
    if i_eject is not None:
        ...
        LV_max_eject_vel_ml_perc = np.nan
        t_LV_eject_pct = np.nan
    ...
    if i_E is not None:
        ...
        LV_E_vel_ml_perc, t_LV_E_pct = np.nan, np.nan
    if i_A is not None:
    ...
    res.update({
        ...
        "LA_LV_coupling_total": (LA_reservoir_EF/LVEF) if (LVEF is not None and not np.isnan(LVEF) and LVEF != 0) else np.nan,
        "LA_LV_coupling_active": (LA_active_EF/LVEF) if (LVEF is not None and not np.isnan(LVEF) and LVEF != 0) else np.nan,
...
def export_patient_plots_and_data(patient_dir: str, metrics: Dict):
    ...
    for tag, key in [("RV eject", "RV_eject"), ("LV eject", "LV_eject"), ("RV E", "RV_E"), ("LV E", "LV_E")]:
        i0 = idx.get(key)
        if i0 is not None:
...
def main():
    ...
    if len(records) == 0:
        raise ValueError("未解析出任何病例（请检查输入表格格式和数据）。")
    ...
    total = df2[id_col].nunique()
    print(f"共检测到 {total} 例（在筛选后按影像号分组）。开始逐例计算...")
    case_idx = 0
    for pid, g in df2.groupby(id_col):
        case_idx += 1
        print(f"[{case_idx}/{total}] 处理 ID={pid} ...")
        chamber_vols: Dict[str, np.ndarray] = {}
        ...
        try:
            ...
            print(f"[警告] 处理右心 ID={pid} 时出错：{e}")
            # 填充 NaN 行，继续
            empty = {"PatientID": canonical_id(pid), "N_phases": len(phase_cols)}
            right_results.append(empty)
        ...
        try:
            ...
            print(f"[警告] 处理左心 ID={pid} 时出错：{e}")
            left_results.append({"PatientID": canonical_id(pid), "N_phases": len(phase_cols)})
